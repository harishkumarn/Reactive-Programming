...
object Async extends AsyncInterface{
  ...
  /**
    ...
    */
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = eventuallyX.map( a => a % 2 == 0)
  ...
  /**
    ...
    */
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] = eventuallyX.recover({case a : Exception => -1})
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation2: () => Future[B]
      ...
      return makeAsyncComputation1().flatMap{	
		    a => makeAsyncComputation2().map(b => (a,b))
	    }
   }
  /**
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] ={
	  if(maxAttempts ==0 ) return Future.failed(new Exception("failed"));
	    return makeAsyncComputation().recoverWith({ case a : Exception => insist(makeAsyncComputation,maxAttempts-1) });
    }
